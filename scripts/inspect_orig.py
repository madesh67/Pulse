...
def analyze_frame_threshold(filepath, threshold=15):
    with Image.open(filepath) as img:
        ...
        for y in range(h):
            for x in range(w):
                ...
                if abs(r - 247) > threshold or abs(g - 247) > threshold or abs(b - 247) > threshold:
                    if x < min_x: min_x = x
                    if x > max_x: max_x = x
                    if y < min_y: min_y = y
                    if y > max_y: max_y = y
...
def main():
    ...
    print("ANALYZING REAL PRODUCT BOUNDS (THRESHOLD = 15):")
    print("-" * 60)
    for idx in landmarks:
        filename = f"frame-{idx:03d}.webp"
        filepath = os.path.join(frames_dir, filename)
        if os.path.exists(filepath):
            bounds = analyze_frame_threshold(filepath, threshold=15)
